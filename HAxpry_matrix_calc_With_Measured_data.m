function [HA_x_matrix_Sensor,HA_pry_matrix_Sensor] = HAxpry_matrix_calc_With_Measured_data(data_Fusion,Sensor_data)
...
%     Hp0 = H1_b.' * Hp1 * hx;
%     Hr0 = H1_b.' * Hr1 * hx;
%     H_matrix0 = [Hy0 Hp0 Hr0];
%     H_matrix0 = pinv(H_matrix0);

    Hy = H1_a * Hy1.' * H1_b * (data_Fusion(ii,:) - Radar_Position_WGS).';
    Hp = H1_a * Hp1.' * H1_b * (data_Fusion(ii,:) - Radar_Position_WGS).';
    Hr = H1_a * Hr1.' * H1_b * (data_Fusion(ii,:) - Radar_Position_WGS).';
    H_matrix2 = [Hy Hp Hr];
    
    
    HA_x_matrix_Sensor(ii).matrix = H_matrix1;
    HA_pry_matrix_Sensor(ii).matrix = H_matrix2;
end
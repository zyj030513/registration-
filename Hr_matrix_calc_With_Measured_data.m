function Hmatrix_Sensor = Hr_matrix_calc_With_Measured_data(Sensor_data)
...
%     Hr = [-cos(r)*sin(y)-sin(r)*sin(p)*cos(y)    -cos(p)*cos(y)   	-sin(r)*sin(y)+cos(r)*sin(p)*cos(y);
%           cos(r)*cos(y)-sin(r)*sin(p)*sin(y)     -cos(p)*sin(y)     sin(r)*cos(y)+cos(r)*sin(p)*sin(y);
%           0                                      0                  0];
    Hr = [-sin(r)*cos(y)-cos(r)*sin(p)*sin(y)    0   	cos(r)*cos(y)-sin(r)*sin(p)*sin(y);
          -sin(r)*sin(y)+cos(r)*sin(p)*cos(y)    0     cos(r)*sin(y)+sin(r)*sin(p)*cos(y);
          -cos(r)*cos(p)                         0     -sin(r)*cos(p)];
    Hmatrix_Sensor(ii).matrix = H1_b.' * Hr * hx;

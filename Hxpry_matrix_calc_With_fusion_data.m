function Hmatrix_Sensor = Hxpry_matrix_calc_With_fusion_data(Sensori_data,data_Fusion)
data_length = length(Sensori_data);
...
for ii = 1 : data_length
    L_Sensor = Sensori_data(ii).Sensor_L;
    B_Sensor = Sensori_data(ii).Sensor_B;
    H_Sensor = Sensori_data(ii).Sensor_H;
    ...
    % 己方传感器的俯仰角、滚转角、偏航角
    p = Sensori_data(ii).Sensor_Attitude(2); % 俯仰角
    r = Sensori_data(ii).Sensor_Attitude(3); % 滚转角
    y = Sensori_data(ii).Sensor_Attitude(1); % 偏航角
...
%     H_matrix1 = inv(H_matrix1);
    %% 姿态角部分
%     hx = W_From_Celiang_to_ENU.' * H1_b * ( Sensori_data(ii).Tar_Posi - Radar_Position_WGS.');

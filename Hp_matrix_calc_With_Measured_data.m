function Hmatrix_Sensor = Hp_matrix_calc_With_Measured_data(Sensor_data)
...
for ii = 1 : data_length
    ...
    Hp = [-sin(r)*cos(p)*sin(y)    sin(p)*sin(y)	cos(r)*cos(p)*sin(y);
          sin(r)*cos(p)*cos(y)     -sin(p)*cos(y)   -cos(r)*cos(p)*cos(y);
          sin(r)*sin(p)            cos(p)           -cos(r)*sin(p)];
    Hmatrix_Sensor(ii).matrix = H1_b.' * Hp * hx;
